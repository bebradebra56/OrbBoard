...
class OrbBoardVi(
    private val orbBoardContext: Context,
    private val orbBoardCallback: OrbBoardCallBack,
    private val orbBoardWindow: Window
) : WebView(orbBoardContext) {
    private var orbBoardFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun orbBoardSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.orbBoardFileChooserHandler = handler
    }
    init {
        ...
        webSettings.apply {
            ...
            userAgentString = WebSettings.getDefaultUserAgent(orbBoardContext).replace("; wv)", "").replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        orbBoardContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(orbBoardContext, "This application not found", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
            }
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("https://test-web.syndi-test.net") != true && url?.contains("https://web.team-s.club") != true) {
                    OrbBoardApplication.orbBoardInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(OrbBoardApplication.ORB_BOARD_MAIN_TAG, "onPageFinished : ${OrbBoardApplication.orbBoardInputMode}")
                    orbBoardWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    orbBoardEnableStableInputScroll()
                } else {
                    OrbBoardApplication.orbBoardInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
                    Log.d(OrbBoardApplication.ORB_BOARD_MAIN_TAG, "onPageFinished : ${OrbBoardApplication.orbBoardInputMode}")
                    orbBoardWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                orbBoardFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            override fun onCreateWindow(
                ...
                orbBoardHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    fun orbBoardFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun orbBoardEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    private fun orbBoardHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView = OrbBoardVi(orbBoardContext, orbBoardCallback, orbBoardWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            orbBoardCallback.orbBoardHandleCreateWebWindowRequest(windowWebView)
        }
    }
